...
**验证手段（新增）**：`npm test` 追加三个离线回归——`scripts/verify-fixes.mjs`（% 文件名 / 符号链接删除语义 / mkdir existOk / 下载错误中文化 / 命令集导出导入与排序 / 上传看护超时，26 断言）、`scripts/verify-drag-paths.mjs`（拖入文件夹的路径映射，9 断言）、`scripts/verify-ansi-html.mjs`（HTML 导出色，10 断言）；另有 **`scripts/verify-security.mjs`（安全回归，47 断言）**：设 `SERVERHUB_TOKEN` 后 REST 无凭据→401（中文 error，写接口同样 401 且不落盘）、头/`?token=`/全大写头名→200、错令牌→401、WS 两条路径无 token→`close 1008` 且带对 token 会真正进业务分支（收到"服务器不存在"）、`SERVERHUB_CORS_ORIGIN` 白名单命中才回 ACAO（且不是 `*`）/未命中不下发/预检命中 204+ACAO、以及**未设令牌时三项"行为不变"**（REST 200、写接口 201、`/ws/local` 正常 connected）。它把"不设令牌=不校验"这条现状契约钉死了：将来若改成默认强制令牌，阶段 B 会失败——这是有意的提醒，需同步文档。**真机回归** `npm run audit:real`（`scripts/real-audit.mjs`；凭据只走 `SH_HOST/SH_USER/SH_PASS` 环境变量，未设置则跳过；只在自己创建的 `/root/serverhub-audit-*` 内增删并清理，不改服务器配置）27 断言：% 文件名、符号链接删除穿透、cwd 采集（含"不误执行半截输入"与前台被 `sleep` 占用时的兜底）、mkdir existOk + 嵌套上传、错误信息、命令集往返、真实分片下的流式解码。
...
**发布到 GitHub（2026-09-11）**：源码推送到 `https://github.com/hwm-1114/ServerHub`（`main`），安装包以 **Release 资产**发布（tag `v1.3.2`，后由 CI 重新出包并发 `v1.3.3`）。**踩坑**：① 本机 git 全局配置了 `http.proxy=http://127.0.0.1:7890`，代理客户端没开时 `git fetch/push` 会直接失败——一次性覆盖即可：`git -c http.proxy= push <url> main`；② 仓库 remote URL 里曾内嵌明文 token（已改为不含 token 的纯 URL，**别再写回去**，改用凭据管理器）；③ `.gitignore` 已排除 `data/`（真实服务器明文密码）、`.verify/`（含凭据的临时脚本）、`release/`、`dist/`——**推送前务必复查 `git status` 确认这三类没被跟踪**。

**发版演练（2026-09-11，v1.3.3 = 首个完全由 CI 直出的版本）**：把 `package.json` 版本改到 1.3.3 → 推 main（run 34658312340 全绿：static/test/package）→ `git tag -a v1.3.3 && git push origin v1.3.3` → 标签触发的 run 34659262289 **四个任务全绿**，`release` 任务创建 Release v1.3.3（非草稿）并上传 `ServerHub.Setup.1.3.3.exe`（109,919,076 字节）+ `.blockmap`，日志里能看到"已创建 Release v1.3.3"与上传后核对资产的两行输出；已复验资产可公开下载（HTTP 200 + 正确 Content-Length）。**至此四个任务都在 CI 上真跑过一遍**：此前唯一没验证的就是这条"标签→发版"路径。**结论**：以后发版只需"改版本号 → 推 main → 打 v* 标签"，不需要在本地出包（CI 出的包已过安装包回归 18 断言）。
...
**CI 首次运行暴露的四个真问题（2026-09-11，已修）**：① `audit:ui` 加载的是**真实前端产物**（server 只在 `dist/` 存在时服务静态文件），而 `test` 任务没构建 → 首页 404、断言集体失败；同时 `ui-audit.cjs` 结尾写死 `app.exit(0)`，**断言失败也会以 0 退出**（本地串跑同样掩盖失败）——已补 `npm run build` 步骤、缺产物明确报错退出 2、退出码改为 `fail ? 1 : 0`。② `stress-process-reliability.mjs` / `stress-switch-stability.mjs` 里 `cls(HELPERS.down(i))` **把 Promise 当函数传**（`await fn()` 抛 `fn is not a function`）：请求变成"发射后不管"，既测不到结果，失败时又成为未处理的 rejection——本地请求都会正常返回所以看不出来，CI 上收尾杀子进程时的 ECONNRESET 才让它以 exit 1 崩掉（而 6 条断言全过）。已改为传 thunk、在 `cls()` 里显式拦截非函数入参、并给该脚本加 `unhandledRejection` 护栏。③ **CI 出包必须显式 `--publish never`**：electron-builder 检测到 CI 环境（`CI=true`）且能从 git remote 推断出 GitHub provider 时，会**隐式尝试把产物发布到 GitHub Release**，没有 `GH_TOKEN` 就直接以 exit 1 结束——而**安装包其实已经生成好了**（日志末尾只有一句 `⨯ GitHub Personal Access Token is not set`）。已抽成 `npm run dist:nsis`（= `electron-builder --win nsis --publish never`）、给 `npm run app` 也加上该参数，并在 `validate:workflow` 里加断言防回归（47 断言）。④ **CDP 目标"URL 已就绪但文档还没提交"的竞态**：`audit:desktop` 在 CI 上偶发"界面已渲染 ❌ url=about:blank"——`/json/list` 在导航**开始**时就把目标 URL 报成 `http://localhost:33120`,而此刻 `location.href` 还是初始的 `about:blank`,固定 `sleep(1500)` 在慢机器上会 snapshot 到空白页(本地机器快,一直没暴露)。已改为轮询 `readyState==='complete' && href 含端口 && #root 有子节点` 再注入 HELPERS(`ui-audit.cjs` 的 `reload()` 同样从固定 sleep 改为等 `#root` 挂载);注意 HELPERS 必须**等页面就绪后再注入**,提前注入会随导航被清掉。
...
**会话重命名修复 + 批量删除（2026-09-22，v1.3.4 发版）**：①会话重命名"很容易失败"的根因是重命名输入框**没有 autoFocus**——菜单点完后焦点仍在别处（常在终端上,直接打字会打进远端 shell）,必须手动点一下输入框才能编辑;补 `autoFocus` + `onFocus` 全选。次生 bug:`handleRenameSession` 不检查 `res.ok`,失败时 `{error}` 响应体整个替换 state 里的会话对象（会话名/标签损坏）,已改为校验+本地拼新名+失败 alert。②文件界面新增**批量勾选删除**：勾选模式工具栏"删除选中(N)"→ ConfirmDialog 输入"删除"二次确认 → 串行 DELETE + 每项失败原因聚合 + 完成清空勾选并刷新。③`audit:ui` 扩到 23 断言(新增重命名 autoFocus/持久化 + 批量删除全流程)。**UI 测试踩坑**：主区 SessionTabs 的标签文本与侧栏会话行相同(都是"会话 N"),按文本找行会误中标签——标签上最后一个按钮是关闭(X),误点直接删会话;必须用侧栏行特有的 `group/ses` 类定位,且 `expandedServers` 在 reload 后重置为折叠,先点服务器行首 chevron 展开。
...
| `npm test` | 一键串跑 11 个离线脚本：五个旧脚本 + `verify-fullhistory` + 可靠性矩阵 51 条 + 批次 J/K/L 的三个离线回归（verify-fixes 26 / verify-drag-paths 9 / verify-ansi-html 10）+ **安全回归 `verify-security.mjs` 47 条**（令牌 401 / WS 1008 / CORS 白名单 / 未设令牌行为不变） |
